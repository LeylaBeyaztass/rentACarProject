...
public class CarManager implements CarService {
	...
	public DataResult<List<ListCarDto>> getByDailyPrice(double dailyPrice) {
		List<Car> cars = this.carDao.getByDailyPrice(dailyPrice);
		List<ListCarDto> response = cars.stream()
				.map(car -> this.modelMapperService.forDto().map(car, ListCarDto.class)).collect(Collectors.toList());
		 return new SuccessDataResult<List<ListCarDto>>(response);
	}
...
//	
//
//Farklı illerde şubeler açmaya karar verdik. Arabalara şehir bilgisi 
//eklenmelidir.
//Şehir bilgisine göre arabalar listelenebilmelidir.
//Kiralamada alış şehri – dönüş şehri bilgisi eklenmelidir.
//
//Dönüş şehri alış şehrinden farklıysa kiralamaya 750 lira ekstra bedel yansıtılır . Toplam tutar
//
//Hizmet kalitemizi arttırmaya karar verdik. Araba kiralanırken ek hizmetler 
//satin alınabilir. Bebek koltuğu, scooter …. Günlük bedelleri üzerinden 
//kiralamaya eklenmelidir
